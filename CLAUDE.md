...
|-------|----------|---------|
| **Claude Code** | オーケストレーション、ユーザー対話 | 全体統括、タスク管理、テスト・lint実行 |
| **Codex CLI** | **実装作業**、深い推論、設計判断 | **コード実装（デフォルト）**、設計相談、デバッグ、リファクタリング |
...
**デフォルト**: 非自明な実装は全てCodexに委譲

- **実装作業**（「〜を実装して」「〜を追加して」）→ サブエージェント経由でCodexに委譲
- 設計判断（「どう実装？」「どのパターン？」）
- デバッグ（「なぜ動かない？」「エラーの原因は？」）
- リファクタリング
- 比較検討（「AとBどちらがいい？」）

**原則**: Claude がオーケストレーション、Codex が実装
