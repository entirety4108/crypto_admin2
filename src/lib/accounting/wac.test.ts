...
describe("WAC ledger", () => {
  it("matches expected fixture (legacy comparison)", () => {
    const snapshots = calculateWacLedger(fixture.transactions as WacTransaction[]);
  ...
  it("recalculates correctly after edit/delete style mutation", () => {
    ...
    const edited: WacTransaction[] = [baseTxs[0]!, { ...baseTxs[1]!, yenAmount: "80.00" }];
  ...
  it("creates cost_basis_history rows from ledger snapshots", () => {
    const rows = buildCostBasisHistoryRows(fixture.transactions as WacTransaction[]);
