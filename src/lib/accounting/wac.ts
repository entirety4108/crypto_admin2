...
const toWacUnits = (totalCostCents: number, totalQtyUnits: number): number => {
  ...
  const raw = (totalCostCents * WAC_FACTOR * QTY_FACTOR) / (JPY_FACTOR * totalQtyUnits);
...
export const calculateWacLedger = (
  ...
  for (const tx of transactions) {
    ...
    snapshots.push({
      ...
      realizedPnl: realizedPnl === null ? null : formatScaledInt(realizedPnl, JPY_SCALE),
    });
  }
...
export const buildCostBasisHistoryRows = (
  ...
  return snapshots.map((snapshot) => {
    ...
    if (!tx) {
      throw new Error(`Transaction not found for snapshot: ${snapshot.transactionId}`);
    }
