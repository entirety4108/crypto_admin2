...
export default async function TransactionsPage({
  ...
  const from = filters.from
    ? new Date(`${filters.from}T00:00:00+09:00`).toISOString()
    : null
  const to = filters.to
    ? new Date(`${filters.to}T23:59:59+09:00`).toISOString()
    : null

  const purchasesQuery = supabase
    .from('purchases')
    .select(
      'id,exec_at,account_id,crypt_id,unit_yen,amount,deposit_yen,purchase_yen,type,commission_id'
    )
    .order('exec_at', { ascending: false })
  const sellsQuery = supabase
    .from('sells')
    .select(
      'id,exec_at,account_id,crypt_id,unit_yen,amount,yen,type,commission_id'
    )
    .order('exec_at', { ascending: false })
  const transfersQuery = supabase
    .from('transfers')
    .select(
      'id,exec_at,from_account_id,to_account_id,crypt_id,amount,commission_id,memo'
    )
    .order('exec_at', { ascending: false })
  ...
  const [
    ...
    supabase
      .from('swaps')
      .select('id,buy_tx_id,sell_tx_id')
  ...
  return (
    <section className="space-y-6">
        ...
        <nav className="-mb-px flex gap-0">
          {tabs.map((tab) => (
            <a
              ...
              className={
                txType === tab.value
                  ? 'border-b-2 border-emerald-500 px-5 py-3 text-sm font-medium whitespace-nowrap text-emerald-600'
                  : 'border-b-2 border-transparent px-5 py-3 text-sm font-medium whitespace-nowrap text-slate-500 hover:text-slate-900'
              }
            >
              {tab.label}
            </a>
      ...
      <form className="flex flex-wrap items-center gap-3" method="get">
        ...
        <button
          type="submit"
          className="inline-flex items-center justify-center rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium hover:bg-slate-50"
        >
          適用
        </button>
      </form>
      ...
      {txType === 'deposit' && (
        <>
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            >
              ...
              >
                ...
                {commissions.map((c) => (
                  <option key={c.id} value={c.id}>
                    {new Date(c.exec_at).toLocaleString('ja-JP')} /{' '}
                    {cryptMap.get(c.crypt_id) ?? 'N/A'}
                  </option>
                ))}
              </select>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <p className="px-6 pt-5 pb-3 text-xs font-semibold tracking-wider text-slate-500 uppercase">
              入金 / エアドロップ 一覧
            </p>
                      ...
                      <TableCell colSpan={9} className="p-0">
                        ...
                        >
                          ...
                          <input
                            type="datetime-local"
                            name="exec_at"
                            defaultValue={toLocalDateTimeInput(tx.exec_at)}
                  ...
                  {swaps.map((s) => (
                    <TableRow key={s.id}>
                      <TableCell className="font-mono text-xs">
                        {s.buy_tx_id.slice(0, 8)}
                      </TableCell>
                      <TableCell className="font-mono text-xs">
                        {s.sell_tx_id.slice(0, 8)}
                      </TableCell>
                      <TableCell>
                    ...
                    <TableRow key={tx.id}>
                      <TableCell colSpan={7} className="p-0">
                        ...
                        >
                          ...
                          <select
                            name="from_account_id"
                            defaultValue={tx.from_account_id}
                            className="rounded border border-slate-200 px-2 py-1 text-xs"
                            required
                          >
                            {accounts.map((a) => (
                          ...
                          <div className="ml-auto flex gap-1">
                            ...
                            <button
                              formAction={deleteTransferAction}
                              type="submit"
                              className="px-2 py-1 text-xs text-red-500 hover:text-red-700"
                            >
                              削除
                    ...
                    <TableRow key={commission.id}>
                      <TableCell colSpan={7} className="p-0">
                        ...
                        >
                          ...
                          <select
                            name="account_id"
                            defaultValue={commission.account_id}
                            className="rounded border border-slate-200 px-2 py-1 text-xs"
                            required
                          >
                            {accounts.map((a) => (
                          ...
                          <select
                            name="crypt_id"
                            defaultValue={commission.crypt_id}
                            className="rounded border border-slate-200 px-2 py-1 text-xs"
                            required
                          >
                            {crypts.map((c) => (
                          ...
                          <input
                            name="unit_yen"
                            defaultValue={commission.unit_yen}
                            type="number"
                            step="0.00000001"
                            min="0"
                            className="w-24 rounded border border-slate-200 px-2 py-1 text-right text-xs"
                          ...
                          <input
                            name="approximate_yen"
                            defaultValue={commission.approximate_yen}
                            type="number"
                            step="0.01"
                            min="0"
                            className="w-24 rounded border border-slate-200 px-2 py-1 text-right text-xs"
