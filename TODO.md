...
## Phase 5: 会計ロジック（WAC）
- [ ] WAC計算ユースケース実装
- [ ] 売却時 realized PnL 計算実装
- [ ] 編集/削除時の再計算実装
- [ ] `cost_basis_history` 更新実装
- [ ] 境界値テスト（小数・ゼロ残高・連続売買）
- [ ] 期待値フィクスチャ作成（旧システム比較用）

## Phase 6: ポートフォリオ・分析
- [ ] ポートフォリオサマリー（総評価額/確定/評価損益）
- [ ] アカウント別保有表示
- [ ] カテゴリ別保有表示
- [ ] 損益レポート（月次/年次）
- [ ] 残高履歴（日次）グラフ
...
## 補助タスク
- [ ] `README.md` を Next.js版運用手順に更新
- [ ] Supabase 型自動生成スクリプト整備
- [ ] 画面ごとの Query Key 命名規約を文書化
- [ ] 日英翻訳キー命名規約を文書化
